    ...
    <div style={{ animation: 'fadeUp .35s ease both', maxWidth: 1040, margin: '0 auto' }}>
            ...
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1 }}>
              <div
                style={{
                  ...
                  fontSize: 15,
                  flex: 'none',
                  background: done ? '#34D399' : active ? 'linear-gradient(135deg,#F05A28,#A56A00)' : 'rgba(255,255,255,.06)',
                  color: done ? '#04160E' : active ? '#fff' : '#6E6E74',
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11, color: '#6E6E74', fontWeight: 600 }}>ÉTAPE {i + 1}</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: active || done ? '#F7F4EF' : '#6E6E74' }}>{label}</div>
              </div>
      ...
      {step === 0 && (
        <div style={{ animation: 'fadeUp .3s ease both' }}>
          <h2 style={{ fontFamily: 'Hanken Grotesk,sans-serif', fontWeight: 600, fontSize: 22, margin: '0 0 4px' }}>Choisis ta niche</h2>
          <p style={{ color: '#8B8B92', margin: '0 0 22px', fontSize: 14 }}>L'IA générera des scripts adaptés à cette thématique, tous les jours.</p>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: selected ? 'rgba(240,90,40,.1)' : 'rgba(255,255,255,.02)',
                    border: `1px solid ${selected ? 'rgba(240,90,40,.5)' : 'rgba(255,255,255,.09)'}`,
                  }}
                >
                  ...
                  <div style={{ fontFamily: 'Hanken Grotesk,sans-serif', fontWeight: 600, fontSize: 16, marginTop: 14 }}>{n.name}</div>
                  <div style={{ fontSize: 12.5, color: '#8B8B92', marginTop: 3, lineHeight: 1.35 }}>{n.desc}</div>
                  {selected && (
                    <span style={{ position: 'absolute', top: 14, right: 14, display: 'flex', color: '#F05A28' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, margin: '30px 0 18px' }}>
            ...
            <span style={{ fontSize: 12, color: '#6E6E74', fontWeight: 600, letterSpacing: '.04em' }}>OU DÉCRIS TON IDÉE</span>
            <span style={{ height: 1, flex: 1, background: 'rgba(255,255,255,.08)' }} />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 18, alignItems: 'start' }}>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 700, marginBottom: 8 }}>
                Ton idée de vidéo <span style={{ color: '#6E6E74', fontWeight: 500 }}>· optionnel</span>
              </label>
              <textarea
                value={videoIdea}
                onChange={(e) => setVideoIdea(e.target.value)}
                placeholder="Ex : 3 erreurs que font les débutants en muscu, ton direct et cash, avec des exemples concrets…"
                style={{ width: '100%', minHeight: 120, background: 'rgba(16,16,18,.75)', border: '1px solid rgba(255,255,255,.12)', borderRadius: 24, padding: '18px 22px', color: '#F7F4EF', fontSize: 14, lineHeight: 1.55, resize: 'vertical' }}
              />
              <div style={{ fontSize: 12, color: '#6E6E74', marginTop: 8, lineHeight: 1.5 }}>💡 Plus tu es précis (angle, ton, exemples, public visé), plus le script collera à ta vision. Cadence combine ton idée avec la niche choisie.</div>
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 13, fontWeight: 700, marginBottom: 8 }}>
                Références <span style={{ color: '#6E6E74', fontWeight: 500 }}>· optionnel</span>
              </label>
              <label
                onDrop={(e) => {
                  e.preventDefault();
                ...
                style={{
                  ...
                  justifyContent: 'center',
                  gap: 8,
                  border: `1.5px dashed ${dragOver ? '#F05A28' : 'rgba(255,255,255,.18)'}`,
                  borderRadius: 16,
                  padding: 26,
                  cursor: 'pointer',
                  textAlign: 'center',
                  background: dragOver ? 'rgba(240,90,40,.08)' : 'rgba(255,255,255,.02)',
                  transition: '.15s',
                }}
              >
                <input type="file" accept="image/*,video/*" multiple onChange={(e) => { addFiles(e.target.files); e.target.value = ''; }} style={{ display: 'none' }} />
                <span style={{ color: '#F05A28', display: 'flex' }}>
                  <Icon name="uploadc" size={22} />
                </span>
                <span style={{ fontSize: 13, fontWeight: 700, color: '#E4E1DA' }}>Glisse des images ou vidéos</span>
                <span style={{ fontSize: 11.5, color: '#6E6E74' }}>ou clique pour parcourir · JPG, PNG, MP4</span>
              </label>
              {refs.length > 0 && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 14 }}>
                  {refs.map((r, i) => (
                    <div key={i} style={{ position: 'relative', width: 74, height: 74, borderRadius: 12, overflow: 'hidden', flex: 'none', border: '1px solid rgba(255,255,255,.12)', background: '#17171B', backgroundImage: r.type === 'image' ? `url(${r.url})` : undefined, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                      {r.type === 'video' && (
                        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, color: '#FF8A5C', background: '#1A1A24', padding: 4 }}>
                          <span style={{ display: 'flex' }}>
                            <Icon name="film" size={20} />
                          </span>
                          <span style={{ fontSize: 8, color: '#8B8B92', maxWidth: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.name}</span>
                        </div>
                      )}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 26 }}>
            <button onClick={next} style={{ background: '#fff', color: '#101012', border: 'none', padding: '12px 26px', borderRadius: 9999, fontFamily: 'Hanken Grotesk', fontWeight: 500, fontSize: 14, cursor: 'pointer', boxShadow: '0 4px 16px rgba(0,0,0,.28)' }}>
              Continuer →
            </button>
          </div>
        </div>
      ...
      {step === 1 && (
        <div style={{ animation: 'fadeUp .3s ease both' }}>
          <h2 style={{ fontFamily: 'Hanken Grotesk,sans-serif', fontWeight: 600, fontSize: 22, margin: '0 0 4px' }}>Réglages de l'automatisation</h2>
          <p style={{ color: '#8B8B92', margin: '0 0 22px', fontSize: 14 }}>Définis le rythme et le style. Modifiable à tout moment.</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
            <div style={{ border: '1px solid rgba(255,255,255,.08)', borderRadius: 16, padding: 20, background: 'rgba(255,255,255,.02)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 14 }}>
                <span style={{ fontFamily: 'Hanken Grotesk,sans-serif', fontSize: 34, fontWeight: 600, color: '#FF8A5C' }}>{perDay}</span>
                <span style={{ color: '#8B8B92', fontSize: 13 }}>vidéo(s) / jour</span>
              </div>
              <input type="range" min={1} max={10} value={perDay} onChange={(e) => setPerDay(+e.target.value)} style={{ width: '100%', accentColor: '#F05A28' }} />
            </div>
            <div style={{ border: '1px solid rgba(255,255,255,.08)', borderRadius: 16, padding: 20, background: 'rgba(255,255,255,.02)' }}>
            ...
            <div style={{ border: '1px solid rgba(255,255,255,.08)', borderRadius: 16, padding: 20, background: 'rgba(255,255,255,.02)', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', marginTop: 'auto' }} onClick={() => setValidate((v) => !v)}>
                <span style={{ position: 'relative', width: 44, height: 25, borderRadius: 20, flex: 'none', transition: 'background .2s', display: 'inline-block', background: validate ? '#F05A28' : 'rgba(255,255,255,.15)' }}>
                ...
                <span>
                  ...
                  <span style={{ display: 'block', fontSize: 12, color: '#8B8B92' }}>Chaque vidéo attend ton feu vert</span>
                </span>
              </label>
            </div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 26 }}>
            <button onClick={prev} style={{ background: 'rgba(255,255,255,.05)', color: '#E4E1DA', border: '1px solid rgba(255,255,255,.1)', padding: '12px 22px', borderRadius: 12, fontFamily: 'Hanken Grotesk', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
              ← Retour
            </button>
            <button onClick={next} style={{ background: '#fff', color: '#101012', border: 'none', padding: '12px 26px', borderRadius: 9999, fontFamily: 'Hanken Grotesk', fontWeight: 500, fontSize: 14, cursor: 'pointer', boxShadow: '0 4px 16px rgba(0,0,0,.28)' }}>
      ...
      {step >= 2 && (
        <div style={{ animation: 'fadeUp .3s ease both', display: 'grid', gridTemplateColumns: '1fr 300px', gap: 32, alignItems: 'start' }}>
          <div>
            <h2 style={{ fontFamily: 'Hanken Grotesk,sans-serif', fontWeight: 600, fontSize: 22, margin: '0 0 4px' }}>{p.done ? 'Ta vidéo est prête 🎬' : 'Génération en cours…'}</h2>
            <p style={{ color: '#8B8B92', margin: '0 0 16px', fontSize: 14 }}>{p.done ? 'Cadence répètera ce processus automatiquement selon ton planning.' : `Pipeline IA · ${overall}% — hook, voix, visuels, montage, export.`}</p>
            {gen.fromIdea && (
              <div style={{ display: 'flex', gap: 11, alignItems: 'flex-start', border: '1px solid rgba(240,90,40,.3)', background: 'rgba(240,90,40,.08)', borderRadius: 12, padding: '12px 14px', marginBottom: 20 }}>
                <span style={{ display: 'flex', color: '#FF8A5C', flex: 'none', marginTop: 1 }}>
                  <Icon name="sparkles" size={16} />
                </span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#FF8A5C', marginBottom: 2 }}>Script basé sur ton idée</div>
                  <div style={{ fontSize: 12.5, color: '#D8D5CE', lineHeight: 1.4 }}>« {videoIdea.trim()} »</div>
                </div>
              </div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {STAGES.map((stg, i) => {
                ...
                return (
                  <div
                    key={stg.key}
                    style={{ display: 'flex', gap: 14, alignItems: 'flex-start', padding: 14, borderRadius: 14, border: `1px solid ${active ? 'rgba(240,90,40,.35)' : 'rgba(255,255,255,.07)'}`, background: active ? 'rgba(240,90,40,.06)' : 'rgba(255,255,255,.015)', transition: '.2s' }}
                  >
                    <div
                      style={{
                        width: 40,
                        ...
                        justifyContent: 'center',
                        transition: '.2s',
                        background: done ? '#34D399' : active ? 'rgba(240,90,40,.2)' : 'rgba(255,255,255,.05)',
                        color: done ? '#04160E' : active ? '#FF8A5C' : '#6E6E74',
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontWeight: 700, fontSize: 14, color: done || active ? '#F7F4EF' : '#8B8B92' }}>{stg.name}</span>
                        <span style={{ fontSize: 12, color: '#8B8B92' }}>{done ? 'Terminé' : active ? 'En cours…' : 'En attente'}</span>
                      </div>
                      <div style={{ fontSize: 12, color: '#8B8B92', marginTop: 1 }}>{stg.desc}</div>
                      <div style={{ height: 5, borderRadius: 10, background: 'rgba(255,255,255,.08)', marginTop: 9, overflow: 'hidden' }}>
                        <div style={{ height: '100%', borderRadius: 10, background: 'linear-gradient(90deg,#F05A28,#A56A00)', width: `${Math.round(p.pcts[i])}%`, transition: 'width .18s linear' }} />
                      </div>
                    </div>
                  </div>
                );
              ...
              <>
                <div style={{ marginTop: 26, border: '1px solid rgba(52,211,153,.3)', background: 'rgba(52,211,153,.08)', borderRadius: 14, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14, animation: 'fadeUp .4s ease both' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12.5, color: '#8B8B92' }}>Ouvre l'éditeur pour peaufiner, ou ajoute-la directement à la file.</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 12, marginTop: 18, flexWrap: 'wrap' }}>
                  <button onClick={addToQueue} style={{ background: '#fff', color: '#101012', border: 'none', padding: '12px 22px', borderRadius: 9999, fontWeight: 500, fontSize: 14, fontFamily: 'Hanken Grotesk', cursor: 'pointer', boxShadow: '0 4px 16px rgba(0,0,0,.28)' }}>
                    Ajouter à la file →
                  </button>
                  <button onClick={editGenerated} style={{ background: 'rgba(255,255,255,.05)', color: '#E4E1DA', border: '1px solid rgba(255,255,255,.1)', padding: '12px 22px', borderRadius: 12, fontWeight: 700, fontSize: 14, fontFamily: 'Hanken Grotesk', cursor: 'pointer' }}>
                    Ouvrir l'éditeur
                  </button>
                  <button onClick={regenerate} style={{ background: 'rgba(255,255,255,.05)', color: '#E4E1DA', border: '1px solid rgba(255,255,255,.1)', padding: '12px 22px', borderRadius: 12, fontWeight: 700, fontSize: 14, fontFamily: 'Hanken Grotesk', cursor: 'pointer' }}>
                    ↻ Régénérer
                  </button>
                </div>
              </>
          ...
          <div style={{ position: 'sticky', top: 96 }}>
            <div style={{ position: 'relative', aspectRatio: '9/16', borderRadius: 24, overflow: 'hidden', border: '1px solid rgba(255,255,255,.12)', background: nicheObj.bg, boxShadow: '0 30px 70px rgba(0,0,0,.5)' }}>
              ...
              <div style={{ position: 'absolute', left: 16, right: 16, bottom: 40, textAlign: 'center', opacity: preview.subs, transition: 'opacity .4s' }}>
                <span style={{ background: '#F05A28', color: '#fff', fontWeight: 800, fontSize: 13, padding: '4px 10px', borderRadius: 6, lineHeight: 1.5 }}>{preview.subLine}</span>
              </div>
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: preview.play, transition: 'opacity .4s', pointerEvents: 'none' }}>
                <span style={{ width: 56, height: 56, borderRadius: '50%', background: 'rgba(255,255,255,.9)', color: '#101012', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 30px rgba(0,0,0,.4)' }}>
                  <Icon name="play" size={22} />
                </span>
              </div>
            </div>
            <div style={{ textAlign: 'center', marginTop: 12, fontSize: 12, color: '#6E6E74' }}>Aperçu en temps réel</div>
          </div>
        </div>
      )}
    </div>
