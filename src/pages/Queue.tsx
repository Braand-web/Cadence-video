    ...
    <div style={{ animation: 'fadeUp .35s ease both', maxWidth: 920 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, border: '1px solid rgba(255,255,255,.08)', borderRadius: 16, padding: '16px 20px', background: 'rgba(255,255,255,.02)', marginBottom: 24 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12.5, color: '#8B8B92' }}>{validate ? 'Activé — les vidéos attendent ta validation avant publication.' : 'Désactivé — publication 100% automatique selon le planning.'}</div>
        </div>
        ...
        return (
          <div key={q.id} style={{ display: 'flex', alignItems: 'center', gap: 16, border: '1px solid rgba(255,255,255,.08)', borderRadius: 14, padding: '12px 14px', marginBottom: 12, background: 'rgba(255,255,255,.02)' }}>
            <div style={{ width: 54, height: 96, borderRadius: 9, overflow: 'hidden', flex: 'none', position: 'relative', background: q.bg, border: '1px solid rgba(255,255,255,.1)' }}>
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center', fontFamily: 'Hanken Grotesk', fontWeight: 700, fontSize: 8, lineHeight: 1.1, padding: 5, color: '#fff', textShadow: '0 1px 4px rgba(0,0,0,.6)' }}>{q.hook}</div>
            </div>
            ...
            {q.status === 'review' && (
              <button onClick={() => approveVideo(q.id)} style={{ background: '#34D399', color: '#04160E', border: 'none', padding: '9px 16px', borderRadius: 10, fontWeight: 700, fontSize: 13, fontFamily: 'Hanken Grotesk', cursor: 'pointer' }}>
                Valider
              </button>
            )}
          </div>
